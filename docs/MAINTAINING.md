...
| `tests/test_regressions.py` | 回归测试(无框架):额度口径 / swarm 分块扫描 / TPS 窗口聚合 / 多会话缓存隔离 / 双 OAuth 凭据槽位与端点解析 / 后台任务段(计数/OSC 8 链接/看板/隐藏/动效剥除) / 看板服务(分组排序/HTTP 端点/段链接形态/幽灵降级/考古过滤/同源 /log/占位提示) / 额度提醒 hook(水位阈值/回显指令/坏输入/install.py Windows 解释器改写) / Windows 适配(detached 参数、stdio UTF-8、安装器行级匹配、doctor OSError 兜底、nt 命令形态)共 79 例,`python3 tests/test_regressions.py` |
...
6. **额度提醒 hook**(v1.6.0,三端):插件 manifest 声明 `UserPromptSubmit` hook(`hooks` 字段,与 config.toml `[[hooks]]` 同 schema;hook 进程 cwd=插件根目录,`./` 相对路径可用,环境变量带 `KIMI_PLUGIN_ROOT`)。stdin 收事件 JSON(`session_id/cwd` 等),exit 0 时 stdout 追加进上下文——**注意:hook_result 只进模型上下文,任何 UI(TUI/桌面端)都不渲染,用户可见只能靠模型回显**(2026-09-17 真机双向实测)。`--hook-quota` 输出 ⏱ 行(只读缓存,绝不同步拉网络);`HOOK_ECHO_H5`/`HOOK_ECHO_WK` 阈值过了才附回显指令。manifest 命令写死 `python3`,Windows 由 install.py `patch_managed_hook()` 改写托管副本为解释器绝对路径。
